...
export async function getTutorBySlug(slug: string): Promise<TutorPublicProfile | null> {
  ...
  try {
    ...
    if (tutorRes.rows.length === 0) return null;
    ...
    for (const row of lessonTypesRes.rows) {
      if (!lessonTypesMap.has(row.id)) {
        lessonTypesMap.set(row.id, { id: row.id, label: row.label, pricing: [] });
      }
    ...
    return { ...tutor, lesson_types: Array.from(lessonTypesMap.values()) };
  } catch (error) {
