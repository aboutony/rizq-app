...
export default function TutorPaymentConfirm({ params }: Params) {
  const locale = ['en','ar','fr'].includes(params?.locale || '') ? params!.locale! : 'en';

  const t = {
    en: { title:'Confirm Payment Received', note:'Student marked payment as completed.', confirm:'Confirm Received', back:'Go Back' },
    ar: { title:'تأكيد استلام الدفع', note:'الطالب أكّد الدفع.', confirm:'تأكيد الاستلام', back:'رجوع' },
    fr: { title:'Confirmer paiement reçu', note:'L’étudiant a marqué le paiement comme effectué.', confirm:'Confirmer', back:'Retour' }
  }[locale as 'en'|'ar'|'fr'];
  ...
  <div class="wrap">
    <div class="card">
      ...
      <p>${t.note}</p>
      <button class="btn" style="margin-top:10px">${t.confirm}</button>
