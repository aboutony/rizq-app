...
export function CancellationForm({ lessonDetails, messages, token }: CancellationFormProps) {
  ...
  const lessonTime = new Date(lessonDetails.confirmed_start_at_utc);
  const now = new Date();
  const hoursUntilLesson = (lessonTime.getTime() - now.getTime()) / (1000 * 60 * 60);
  const isLate = hoursUntilLesson < lessonDetails.cutoff_hours;
    ...
    <div className="bg-white p-8 rounded-lg shadow-md border text-center">
        ...
        {isLate && (
            <div className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-6" role="alert">
                <p className="font-bold">{messages.parent_flow.cancellation.late_warning.replace('{hours}', lessonDetails.cutoff_hours.toString())}</p>
            </div>
        )}
